                                                                ...
                                                                const MoeInitRoutingV3Arch35TilingData *tilingData)
{
    ...
    colsQuant_ = (cols_ + 1) / FP4_NUMBER;
    perLoopColsQuant_ = (perLoopCols_ + 1) / FP4_NUMBER;
                                                             ...
                                                             int64_t loopScaleCols)
{
    ...
    int64_t loopColsQuant = (loopCols + 1) / FP4_NUMBER;

    DataCopyExtParams copyOutParams = {1, static_cast<uint32_t>(loopColsQuant * sizeof(uint8_t)), 0, 0, 0};
    DataCopyPad<uint8_t>(expandedXAndScaleOutGm_[dstIdx * (colsQuant_ + scaleCols_) +
                        colIdx * perLoopColsQuant_], outLocal, copyOutParams);

    DataCopyExtParams copyScaleParams = {1, static_cast<uint32_t>(loopScaleCols * sizeof(uint8_t)), 0, 0, 0};
    DataCopyPad<uint8_t>(expandedXAndScaleOutGm_[dstIdx * (colsQuant_ + scaleCols_) +
                        colsQuant_ + colIdx * perLoopScaleCols_], mxScaleLocal, copyScaleParams);
