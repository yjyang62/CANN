...
/*!
 * \file quant_reduce_scatter_v2_proto.h
 * \brief 图模式原型定义
 */
#ifndef QUANT_REDUCE_SCATTER_V2_PROTO_H_
#define QUANT_REDUCE_SCATTER_V2_PROTO_H_
 ...
 * @li context: A tensor. Support dtype: int32, dimension must be 1, Support Shape (272, ), support format: ND.
 * @li x: A matrix tensor. The type support int8, hifloat8, float8_e4m3fn, float8_e5m2, float4_e1m2, float4_e2m1.
 ...
 * @li output_dtype: An optional int identifying the data type of output.
 *                   The type support 0(float), 1(float16), 27(bfloat16). Default: 27(bfloat16).
 * @li world_size: A required int identifying the rank size.
 */
REG_OP(QuantReduceScatterV2)
    .INPUT(context, "T0")
    .INPUT(x, "T1")
    .INPUT(scales, "T2")
    .OUTPUT(out_put, "T3")
    .DATATYPE(T0, TensorType({DT_INT32}))
    ...
    .REQUIRED_ATTR(hccl_buffer_size, Int)
    .ATTR(reduce_op, String, "sum")
    .ATTR(output_dtype, Int, DT_BF16)
    .REQUIRED_ATTR(world_size, Int)
    .OP_END_FACTORY_REG(QuantReduceScatterV2)

} // namespace ge

#endif // QUANT_REDUCE_SCATTER_V2_PROTO_H_
