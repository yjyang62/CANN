...
/*!
 * \file inplace_rotate_interleaved_split_s_pad_mixed.h
 * \brief
 */
#ifndef INPLACE_ROTATE_INTERLEAVED_SPLIT_S_PAD_MIXED_H
#define INPLACE_ROTATE_INTERLEAVED_SPLIT_S_PAD_MIXED_H
...
template <typename T>
__aicore__ inline void InterleavedSplitSPadMixed<T>::InitData(const InplacePartialRopeRegbaseTilingData *tiling)
{
    tiling_ = tiling;
    ...
    triHeadDimAlign = (headDim + triBlockNum - 1) / triBlockNum * triBlockNum;
}
    ...
    uint32_t loopIdx, uint32_t calcLen)
{
    ...
    // Reserve one SND row for later broadcast; subtract the float-aligned tri data already copied.
    dataCopyTriParams.dstStride = static_cast<uint16_t>(
        (numHeads * headDimAlign - triHeadDimAlign) / triBlockNum);
...
#endif // INPLACE_ROTATE_INTERLEAVED_SPLIT_S_PAD_MIXED_H
