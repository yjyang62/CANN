...
/*!
 * \file inplace_rotate_interleaved_split_s_pad.h
 * \brief
 */
...
template <typename T>
__aicore__ inline void InterleavedSplitSPad<T>::InitData(const InplacePartialRopeRegbaseTilingData *tiling)
{
    tiling_ = tiling;
...
__aicore__ inline void InterleavedSplitSPad<T>::CopyInX(LocalTensor<T> &x, uint32_t loopIdx, uint32_t calcLen)
{
    ...
    for (uint32_t batchIdx = 0; batchIdx < batchSize; ++batchIdx) {
        ...
        DataCopyPad(x[batchIdx * calcLen * numHeads * headDimAlign],
                    xGm[batchIdx * seqLen * bufferNdSizeAll + loopIdx * ubCalcSeq * bufferNdSizeAll + start],
                        dataCopyParams,
                    {false, 0, 0, 0});
    }
