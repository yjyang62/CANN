...
安装完成后，您可以像使用普通PyTorch操作一样使用NPU算子，以FIA算子为例，您可以在`ascend_ops/csrc/flash_attn_example/test`目录下找到并执行这个脚本:

```bash
cd ./ascend_ops/csrc/flash_attn_example/test/
python3 test_fa.py
```
...
- [FA算子实践](./ascend_ops/csrc/flash_attn_example/README.md)。
