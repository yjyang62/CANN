...
/*!
 * \file block_epilogue_dequant_finalize_routing_deter.h
 * \brief Deterministic variant of BlockEpilogueDequantFinalizeRouting
 *        Uses linear workspace write instead of atomic scatter, with sliding window flush.
 */

#ifndef BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_H
...
namespace Block {
namespace {
constexpr uint32_t Y_IDXD = 0U;
constexpr uint32_t BIAS_IDXD = 1U;
constexpr uint32_t X2SCALE_IDXD = 2U;
constexpr uint32_t X1SCALE_IDXD = 3U;
constexpr uint32_t LOGIT_IDXSD = 4U;
constexpr uint32_t MAX_SINGLE_MNSD = 128 * 256U;
constexpr uint32_t HALF_DB_MAX_SINGLE_MNSD = 32 * 256U;
...
class BlockEpilogueDequantFinalizeRoutingDeter {
public:
    __aicore__ inline BlockEpilogueDequantFinalizeRoutingDeter();
    __aicore__ inline ~BlockEpilogueDequantFinalizeRoutingDeter();
...
public:
    ...
    __aicore__ inline void DeterministicFlushRow(uint64_t mOffset, uint64_t outRow, uint64_t nSize);
...
private:
    ...
    __aicore__ inline void LinearWriteToWorkspace(uint32_t curBaseN, uint32_t curVecBaseM, uint64_t offsetM,
    ...
    AscendC::GlobalTensor<DataTypeOut> mmQuantOutGm_;

    // UB ADDR
    AscendC::LocalTensor<DataTypeIn> l0cOutUb_{AscendC::TPosition::VECIN, DETER_UB_SIZED, MAX_SINGLE_MNSD};
    AscendC::LocalTensor<float> l0cOutUbFloat_{AscendC::TPosition::VECIN, DETER_UB_SIZED, MAX_SINGLE_MNSD};
    ...
    AscendC::TQueBind<AscendC::TPosition::VECIN, AscendC::TPosition::VECOUT, 1> deterDmaQue_;
    ...
    uint64_t groupAccumM_ = 0;
};

GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::BlockEpilogueDequantFinalizeRoutingDeter()
{
    if ASCEND_IS_AIV {
...
BlockEpilogueDequantFinalizeRoutingDeter<GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::Init(
    Params const &params, AscendC::TPipe *pipe)
{
    if ASCEND_IS_AIC {
        return;
    }
    ...
    uint32_t afterFirstIn = DETER_UB_SIZED + MAX_SINGLE_MNSD * sizeof(DataTypeOut);
    logitUbPing_ = AscendC::LocalTensor<float>(AscendC::TPosition::VECIN, afterFirstIn, BLOCKS_BYTESD);
    uint32_t afterSecondIn = afterFirstIn + BLOCKS_BYTESD * sizeof(float);
    logitUbPong_ = AscendC::LocalTensor<float>(AscendC::TPosition::VECIN, afterSecondIn, BLOCKS_BYTESD);
    uint32_t afterLogit = afterSecondIn + BLOCKS_BYTESD * sizeof(float);
    x2ScaleUbPing_ = AscendC::LocalTensor<DataTypeX2Scale>(AscendC::TPosition::VECIN, afterLogit, BLOCKS_BYTESD);
    uint32_t afterX2ScalePing = afterLogit + BLOCKS_BYTESD * sizeof(DataTypeX2Scale);
    x2ScaleUbPong_ = AscendC::LocalTensor<DataTypeX2Scale>(AscendC::TPosition::VECIN, afterX2ScalePing, BLOCKS_BYTESD);
    uint32_t afterX2ScalePong = afterX2ScalePing + BLOCKS_BYTESD * sizeof(DataTypeX2Scale);
    x1ScaleUbPing_ = AscendC::LocalTensor<DataTypeX1Scale>(AscendC::TPosition::VECIN, afterX2ScalePong, BLOCKS_BYTESD);
    uint32_t afterX1ScalePing = afterX2ScalePong + BLOCKS_BYTESD * sizeof(DataTypeX1Scale);
    x1ScaleUbPong_ = AscendC::LocalTensor<DataTypeX1Scale>(AscendC::TPosition::VECIN, afterX1ScalePing, BLOCKS_BYTESD);
    uint32_t afterX1ScalePong = afterX1ScalePing + BLOCKS_BYTESD * sizeof(DataTypeX1Scale);
    biasUbPing_ = AscendC::LocalTensor<BiasDtype>(AscendC::TPosition::VECIN, afterX1ScalePong, BLOCKS_BYTESD);
    uint32_t afterBiasPing = afterX1ScalePong + BLOCKS_BYTESD * sizeof(BiasDtype);
    biasUbPong_ = AscendC::LocalTensor<BiasDtype>(AscendC::TPosition::VECIN, afterBiasPing, BLOCKS_BYTESD);
    uint32_t afterBiasPong = afterBiasPing + BLOCKS_BYTESD * sizeof(BiasDtype);
    outUbPing_ = AscendC::LocalTensor<DataTypeOut>(AscendC::TPosition::VECOUT, afterBiasPong, HALF_DB_MAX_SINGLE_MNSD);
    outUbPong_ = AscendC::LocalTensor<DataTypeOut>(AscendC::TPosition::VECOUT,
                                                   afterBiasPong + HALF_DB_MAX_SINGLE_MNSD * sizeof(DataTypeOut),
                                                   HALF_DB_MAX_SINGLE_MNSD);
...
GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline auto BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::GetL0c2UbTensor()
{
    return l0cOutUb_;
}
...
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::UpdateGlobalAddr(const BlockCoord &baseOffset)
{
    if ASCEND_IS_AIV {
        logitGlobal_.SetGlobalBuffer((__gm__ float *)params_->logitGmAddr + Get<LOGIT_IDXSD>(baseOffset));
        if (params_->x1ScaleGmAddr != nullptr) {
            x1ScaleGlobal_.SetGlobalBuffer((__gm__ DataTypeX1Scale *)params_->x1ScaleGmAddr +
                                           Get<X1SCALE_IDXD>(baseOffset));
        }
        x2ScaleGlobal_.SetGlobalBuffer((__gm__ DataTypeX2Scale *)params_->x2ScaleGmAddr +
                                       Get<X2SCALE_IDXD>(baseOffset));
        if (params_->biasGmAddr != nullptr) {
            biasGlobal_.SetGlobalBuffer((__gm__ BiasDtype *)params_->biasGmAddr + Get<BIAS_IDXD>(baseOffset));
        }
    }
}
...
GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::CopyInLogit(uint32_t curBaseM,
                                                                                      uint64_t offsetM,
                                                                                      LocalTensor<float> &logitUb)
{
...
__aicore__ inline void
BlockEpilogueDequantFinalizeRoutingDeter<GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::
    LinearWriteToWorkspace(uint32_t curBaseN, uint32_t curVecBaseM, uint64_t offsetM, uint64_t yOffset,
...
GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::VFDoLogitMuls(uint32_t offsetRe,
                                                                                        uint32_t offsetLogit,
                                                                                        uint16_t repeatTimesLogit,
                                                                                        uint16_t repeatTimesRe,
...
__aicore__ inline void
BlockEpilogueDequantFinalizeRoutingDeter<GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::
    VFDoDequantOnlyX2(__ubuf__ float *dst, __ubuf__ DataTypeIn *l0cOut, __ubuf__ DataTypeX2Scale *x2Scale,
                      __ubuf__ BiasDtype *bias, uint16_t mSize, uint16_t nSize)
{
...
template <bool isBiasEpilogue>
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::VFDoDequant(__ubuf__ float *dst,
                                                                                      __ubuf__ DataTypeIn *l0cOut,
                                                                                      __ubuf__ DataTypeX2Scale *x2Scale,
                                                                                      __ubuf__ DataTypeX1Scale *x1Scale,
                                                                                      __ubuf__ BiasDtype *bias,
                                                                                      uint16_t mSize, uint16_t nSize)
{
    uint32_t eleNumPerVf = AscendC::VECTOR_REG_WIDTH / sizeof(DataTypeIn);
    uint32_t nSrcUbAligned = Align(nSize, static_cast<uint16_t>(UB_ALIGN_SIZE / sizeof(DataTypeIn)));
    ...
    CopyX1ScaleFromGm2Ub(LocalTensor<DataTypeX1Scale> &dst, uint64_t blockLen, uint64_t offset)
{
    ...
    AscendC::DataCopyPad(dst, x1ScaleGlobal_[Get<X1SCALE_IDXD>(blockCoord_) + offset], ptScale2UbParams, padParams);
}
    ...
    CopyX2ScaleFromGm2Ub(LocalTensor<DataTypeX2Scale> &dst, uint64_t offset)
{
    ...
    AscendC::DataCopyPad(dst, x2ScaleGlobal_[Get<X2SCALE_IDXD>(blockCoord_) + offset], scale2UbParams, padParams);
}

GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::CopyBiasFromGm2Ub(LocalTensor<BiasDtype> &dst)
{
    DataCopyParams bias2UbParams{1, 0, 0, 0};
    DataCopyPadParams padParams;
    bias2UbParams.blockLen = singleN_ * sizeof(BiasDtype);
    AscendC::DataCopyPad(dst, biasGlobal_[Get<BIAS_IDXD>(blockCoord_)], bias2UbParams, padParams);
}

GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline uint64_t BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::GetRowIndex(uint64_t globalOffset)
{
...
GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::SetWorkspaceGroupOffset(uint64_t groupAccumM)
{
...
GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::DeterministicFlushRow(uint64_t mOffset,
                                                                                                uint64_t outRow,
                                                                                                uint64_t nSize)
{
    ...
    for (uint64_t nOff = 0; nOff < nSize; nOff += baseN) {
        ...
        DataCopyPad(local, mmQuantOutGm_[mOffset * nSize + nOff], copyParams, DataCopyPadExtParams<DataTypeOut>{});
...
GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_CLASS_LOCAL_PARAMS
__aicore__ inline void BlockEpilogueDequantFinalizeRoutingDeter<
    GMM_BLOCK_EPILOGUE_DEQUANT_FINALIZE_ROUTING_DETER_FUNC_LOCAL_PARAMS>::operator()(const BlockShape &blockShape,
                                                                                     const BlockCoord &blockCoord)
{
    ...
    alignN_ = CeilDiv(singleN_, BLOCK_ELEMENTS_FP32SD) * BLOCK_ELEMENTS_FP32SD;
    uint64_t singleMInVec = subBlockIdx_ == 1 ? singleM_ - halfSingleM : halfSingleM;
    if (singleMInVec == 0) {
        return;
    }
    uint64_t mOffset = subBlockIdx_ * halfSingleM;
    vlForFloatNumber_ = BLOCKS_BYTESD / sizeof(DataTypeOut);
    auto repeatTimesRe = CeilDiv(singleN_, vlForFloatNumber_);
    uint64_t logitOffset = Get<LOGIT_IDXSD>(blockCoord) + mOffset;
    uint64_t yOffset = Get<Y_IDXD>(blockCoord);
    uint16_t remainRepeatTimesLogit =
        (singleMInVec % MAX_OUTPUT_M_UBSD != 0) ? singleMInVec % MAX_OUTPUT_M_UBSD : MAX_OUTPUT_M_UBSD;
    ...
    uint32_t loopNumY = CeilDiv(singleMInVec, MAX_OUTPUT_M_UBSD);
    for (uint32_t i = 0; i < loopNumY; i++) {
        AscendC::WaitFlag<AscendC::HardEvent::MTE3_V>(yPingPongID_);
        repeatTimesLine_ = (i == loopNumY - 1) ? remainRepeatTimesLogit : MAX_OUTPUT_M_UBSD;
        ...
        VFDoLogitMuls(i * MAX_OUTPUT_M_UBSD * alignN_, i * MAX_OUTPUT_M_UBSD, repeatTimesLine_, repeatTimesRe,
                      outUbAddr, logitUb);
