...
/*!
 * \file test_geir_kv_compress_epilog.cpp
 * \brief GE IR (graph mode) RunGraph test case for KvCompressEpilog.
 */
...
#include "nn_other.h"
#include "../op_graph/kv_compress_epilog_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                    \
    ...
    kv_compress_epilog_op.set_input_##intputName(placeholder##intputIndex);                                            \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) kv_compress_epilog_op.set_attr_##attrName(attrValue);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                                  \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype);            \
    kv_compress_epilog_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
                     ...
                     std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto kv_compress_epilog_op = op::KvCompressEpilog("test_geir_kv_compress_epilog");

    // shape 定义（quant_mode=1）
    // x: [tokenNum, d]，d=256 满足 d%64==0 且 >64
    // cache: 4D [blockNum, blockSize, 1, headDim]（4D-only 契约，倒数第二维固定为 1）。
    //   num_slots = blockNum*blockSize = 128*16 = 2048; headDim=384 >= 算子写出的 kvCacheCol
    //   (mode1 d=256 -> concatCol=323, E8M0 不补齐, kvCacheCol=323 <= 384)。
    // slot_mapping: [tokenNum]
    const int64_t tokenNum = 1024;
    const int64_t d = 256;
    const int64_t blockNum = 128;
    const int64_t blockSize = 16;
    const int64_t headDim = 384;
    std::vector<int64_t> cacheShape = {blockNum, blockSize, 1, headDim};
    std::vector<int64_t> xShape = {tokenNum, d};
    std::vector<int64_t> slotMappingShape = {tokenNum};

    // 添加输入（顺序严格匹配 proto.h: cache, x, slot_mapping）
    ADD_INPUT(1, cache, DT_UINT8, cacheShape);
    ADD_INPUT(2, x, DT_BF16, xShape);
    ADD_INPUT(3, slot_mapping, DT_INT32, slotMappingShape);

    // 添加输出（cache 原地更新）
    ADD_OUTPUT(1, cache, DT_UINT8, cacheShape);

    // 添加属性（quant_mode=1: nope per-group FP8 dynamic quant, scale 存为 float8_e8m0）
    ADD_INPUT_ATTR(quant_group_size, (int64_t)64);
    ADD_INPUT_ATTR(quant_mode, (int64_t)1);
    ADD_INPUT_ATTR(round_scale, true);
    ADD_INPUT_ATTR(x_scale, 1.0f);

    outputs.push_back(kv_compress_epilog_op);
    // 添加完毕
    return SUCCESS;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        for (int64_t j = 0; j < 16 && j < output_shape; j++) {
            LOG_PRINT("output[0] byte[%ld] is: %u\n", j, (uint32_t)output_data_i[j]);
        }
    }
