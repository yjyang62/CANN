...
#include "moe_w4a16_single.h"
#include <cmath>

using namespace AscendC;

template <typename T>
__aicore__ inline void MOEW4A16Single<T>::Init(GM_ADDR x, GM_ADDR quantized_weight_13, GM_ADDR weight_scale_13,
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::InitEventId()
{
    if ASCEND_IS_AIV {
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::InitWorkspaceTensors(GM_ADDR usrWorkspace)
{
...
MOEW4A16Single<T>::InitGlobalTensors(GM_ADDR x, GM_ADDR quantized_weight_13, GM_ADDR weight_scale_13,
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::InitLocalTensors()
{
    if ASCEND_IS_AIV {
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::PrepareMMCommon()
{
    if ASCEND_IS_AIV {
        uint32_t baseNumber = tilingData->L0ASize / 2 / sizeof(T);
        ubWInt4 = ubBase.ReinterpretCast<int4b_t>();
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::ComputeMMTiling()
{
    mmAGm = MM2InputGm;
    mmCGm = MM2OutputGm;
    mmBGm0 = wGm_2;
    ...
    baseNNum = (fracN + 16 - 1) / 16;
    uint32_t baseN = (fracN + baseNNum - 1) / baseNNum;
    uint32_t baseMLimit = 128 / baseN;
    uint32_t baseKLimit = 64 / (baseN > baseMLimit ? baseN : baseMLimit);
    baseKNum = (fracK + baseKLimit - 1) / baseKLimit;
    totalTaskNum = tilingData->topK * baseNNum;
...
template <>
__aicore__ inline void MOEW4A16Single<half>::AntiQuantWeight(uint32_t task_id, uint32_t ping_pong, bool endKFlag,
...
template <>
__aicore__ inline void MOEW4A16Single<bfloat16_t>::AntiQuantWeight(uint32_t task_id, uint32_t ping_pong, bool endKFlag,
                                                                   uint32_t baseN, uint32_t baseK, uint32_t offsetN,
                                                                   uint32_t offsetK)
{
    uint32_t scaleBegin = offsetK * 16 / tilingData->scaleGroupSize;
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::ComputeMatMul(uint32_t task_id, uint32_t ping_pong, bool startKFlag,
                                                        bool endKFlag, bool needSetFlag, uint32_t realM, uint32_t baseM,
                                                        ...
                                                        uint32_t offsetN, uint32_t offsetK)
{
    ...
    if (endKFlag) {
        ...
        repeatParamsC.mSize = 1;
        repeatParamsC.dstStride = fracN * 16;
        repeatParamsC.srcStride = 1 * 16;
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::ProcessMM()
{
    ...
    for (int32_t i = 0; i < tilingData->originE; i++) {
        if (realM[i] == 0)
            continue;
        rowIdxRev[tmp++] = realM[i] - 1;
        if ((task_id + (baseKNum * baseNNum)) <= MMtaskOffset || task_id >= (MMtaskOffset + MMtaskNum)) {
            if (!firstMM)
                mmAGm = mmAGm[fracK * 16];
            mmCGm = mmCGm[fracN * 16];
            task_id += (baseKNum * baseNNum);
            continue;
        }
            ...
            for (int32_t l = 0; l < baseKNum; l++) {
                ...
                if ASCEND_IS_AIV {
                    if (subblock_id == (inner_task_id % 2)) {
                        bool endK =
                            (task_id == (MMtaskOffset + MMtaskNum - 1)) || (task_id == (MMtaskOffset + MMtaskNum - 2));
                        ...
                        ffts_cross_core_sync(
                            PIPE_MTE3, GetffstMsg(0x02, ping_pong ? SYNC_PING_AIV_AIC_FLAG : SYNC_PONG_AIV_AIC_FLAG));
                    }
                }
                if ASCEND_IS_AIC {
                    ...
                    ComputeMatMul(inner_task_id, ping_pong, startK, endK, needSetFlag, 1, realbaseM, realbaseN,
                                  realbaseK, offsetM, offsetN, offsetK);
                }
                task_id++;
                offsetK += realbaseK;
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::ProcessCountGroup()
{
    for (int i = 0; i < tilingData->topK; i++) {
        uint32_t idx = expertIdxGm.GetValue(i);
        realM[idx] = i + 1;
    }
}
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::ComputeRouteMMTiling()
{
    mmAGm = xGm;
    mmCGm = MM1OutputGm;
    mmBGm0 = wGm_13;
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::PrepareSwigluCommon()
{
    if ASCEND_IS_AIV {
...
template <typename T>
template <typename U>
__aicore__ inline void MOEW4A16Single<T>::SwishFunc(const LocalTensor<U> &dst, const LocalTensor<U> &src)
{
    BinaryRepeatParams binaryParams;
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::ProcessSwiglu()
{
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::PrepareReRouteCommon()
{
    if ASCEND_IS_AIV {
...
template <>
__aicore__ inline void MOEW4A16Single<bfloat16_t>::ProcessReRoute()
{
    ...
    for (int32_t i = 0; i < taskN; i++) {
        ...
        DataCopy<bfloat16_t>(ubReRouteRes[pingpong1], MM2OutputGm[offsetN], repeatParams);
        SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
        ...
        Muls<float, false>(ubReRouteResFloat[pingpong1], ubReRouteResFloat[pingpong1],
                           ToFloat(topKwGm.GetValue(rowIdxRev[0])), MASK_PLACEHOLDER, 1, unaryParams);

        for (int32_t j = 1; j < tilingData->topK; j++) {
            pingpong2 = !pingpong2;
            WaitFlag<HardEvent::V_MTE2>(eventIdVToMTE2[pingpong2]);
            DataCopy<bfloat16_t>(ubReRouteBuf[pingpong2], MM2OutputGm[j * tilingData->originN_2 + offsetN],
                                 repeatParams);
            SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
            ...
            Muls<float, false>(ubReRouteBufFloat[pingpong2], ubReRouteBufFloat[pingpong2],
                               ToFloat(topKwGm.GetValue(rowIdxRev[j])), MASK_PLACEHOLDER, 1, unaryParams);
...
template <>
__aicore__ inline void MOEW4A16Single<half>::ProcessReRoute()
{
    DataCopyParams repeatParams;
    BinaryRepeatParams binaryParams;
    UnaryRepeatParams unaryParams;
    ...
    for (int32_t i = 0; i < taskN; i++) {
        ...
        repeatParams.blockLen = realBaseN / 16;
        WaitFlag<HardEvent::MTE3_MTE2>(eventIdMTE3ToMTE2[pingpong1]);
        DataCopy<half>(ubReRouteRes[pingpong1], MM2OutputGm[offsetN], repeatParams);
        SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);

        WaitFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
        SetVectorMask<half, MaskMode::COUNTER>(realBaseN);
        Muls<half, false>(ubReRouteRes[pingpong1], ubReRouteRes[pingpong1], topKwGm.GetValue(rowIdxRev[0]),
                          MASK_PLACEHOLDER, 1, unaryParams);

        for (int32_t j = 1; j < tilingData->topK; j++) {
            pingpong2 = !pingpong2;
            WaitFlag<HardEvent::V_MTE2>(eventIdVToMTE2[pingpong2]);
            DataCopy<half>(ubReRouteBuf[pingpong2], MM2OutputGm[j * tilingData->originN_2 + offsetN], repeatParams);
            SetFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);

            WaitFlag<HardEvent::MTE2_V>(eventIdMTE2ToV[0]);
            Muls<half, false>(ubReRouteBuf[pingpong2], ubReRouteBuf[pingpong2], topKwGm.GetValue(rowIdxRev[j]),
                              MASK_PLACEHOLDER, 1, unaryParams);
...
template <typename T>
__aicore__ inline void MOEW4A16Single<T>::Process()
{
    // step 1
    ProcessCountGroup();
    // step 2
