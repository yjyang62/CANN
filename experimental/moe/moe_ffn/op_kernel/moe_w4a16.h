 ...
 * \brief
 */
#ifndef _ASCENDC_MOE_W4A16_H_
#define _ASCENDC_MOE_W4A16_H_
...
protected:
    ...
    __aicore__ inline void PrepareRouteCommon();
    __aicore__ inline void ProcessRoute();
    ...
    uint32_t scaleK;
    uint32_t baseKNum;
    uint32_t baseNNumA, baseNNumB;
    uint32_t baseMLimitA;
    uint32_t baseMNum[GROUP_MAX];
    ...
    LocalTensor<half> ubWHalf, ubWHalf_;
    LocalTensor<float> ubWFloat, ubWFloat_;
    LocalTensor<T> ubWT;
    ...
    GlobalTensor<T> routedGm;
    uint64_t ZNLocalList[2][16], NDLocalList[2][16];

    LocalTensor<T> ubRouteND[2];
    LocalTensor<T> ubRouteZN[2];
    // swiglu
    ...
    LocalTensor<int32_t> rowIdxUbRev;
    LocalTensor<T> topKwUb;
};
#endif
