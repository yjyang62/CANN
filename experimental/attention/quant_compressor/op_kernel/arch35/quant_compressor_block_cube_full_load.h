...
/*!
 * \file quant_compressor_block_cube_full_load.h
 * \brief
 */

#ifndef QUANT_COMPRESSOR_BLOCK_CUBE_FULL_LOAD_H
#define QUANT_COMPRESSOR_BLOCK_CUBE_FULL_LOAD_H
...
class QuantCompressorBlockCubeFullLoad {
    using MM1_OUT_T = float;

public:
    __aicore__ inline QuantCompressorBlockCubeFullLoad(){};
...
private:
    using T = float;
    using X_T = hifloat8_t;
    static constexpr uint32_t NZ_C0_SIZE = 32 / sizeof(X_T);
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::InitParams(const ConstInfo &constInfo,
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::Init(
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::InitBuffers(TPipe *pipe)
{
    // L1
...
QuantCompressorBlockCubeFullLoad<COMP>::InitGlobalBuffers(const GlobalTensor<MM1_OUT_T> &kvMm1ResGm,
                                                          const GlobalTensor<MM1_OUT_T> &scoreMm1ResGm)
{
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::AllocEventID(TPipe *pipe)
{
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::FreeEventID(TPipe *pipe)
{
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::CopyXGmToL1(LocalTensor<X_T> xL1Tensor, uint32_t hIdx,
                                                                           uint32_t kBase)
{
    ...
    uint32_t dstNzC0Stride = Align(copySeqCnt, 16U); // 1行变2行的行方向的偏移，需要16对齐
    CopySingleMatrixNDToNZ(xL1Tensor[xL1Offset], xGm_[gmOffset], nValue, dValue, srcDValue, dstNzC0Stride);
}

template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::CopyWeightGmToL1(LocalTensor<X_T> wL1Tensor,
...
template <typename COMP>
__aicore__ inline void
QuantCompressorBlockCubeFullLoad<COMP>::LoadAToL0(LocalTensor<X_T> aL0Tensor, LocalTensor<X_T> xL1Tensor,
...
template <typename COMP>
__aicore__ inline void
QuantCompressorBlockCubeFullLoad<COMP>::LoadBToL0(LocalTensor<X_T> bL0Tensor, LocalTensor<X_T> wL1Tensor,
...
template <typename COMP>
__aicore__ inline void
QuantCompressorBlockCubeFullLoad<COMP>::MatrixMmad(LocalTensor<T> cL0Tensor, LocalTensor<X_T> aL0Tensor,
...
template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::CopyOutMm1Res(const RunInfo &info,
...
template <typename COMP>
__aicore__ inline uint32_t QuantCompressorBlockCubeFullLoad<COMP>::GetMSize(const RunInfo &info, uint32_t coffId)
{
    return info.dealSeqCnt;
}

template <typename COMP>
__aicore__ inline void QuantCompressorBlockCubeFullLoad<COMP>::ComputeMm1(const RunInfo &info)
{
...
#endif // QUANT_COMPRESSOR_BLOCK_CUBE_FULL_LOAD_H