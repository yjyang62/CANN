...
__simd_vf__ void ProcessVec1UpdateImpl128VF(
    __ubuf__ T2 * expUb,  __ubuf__ T * srcUb, __ubuf__ T * inMaxUb, __ubuf__ T * tmpExpSumUb,
    __ubuf__ T * tmpMaxUb, __ubuf__ T * tmpMaxUb2, __ubuf__ uint8_t *indexesUb, const uint32_t blockStride,
    const uint32_t repeatStride, const uint16_t m, const T scale, const T minValue)
{
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (IsSameType<T2, bfloat16_t>::value) {
            ...
            // multiply 16: 16 for MLA FULL QUANT
