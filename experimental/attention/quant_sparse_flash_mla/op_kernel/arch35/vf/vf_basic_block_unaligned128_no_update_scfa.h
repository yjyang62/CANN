    ...
    const uint16_t m, const T scale, const T minValue, uint32_t pltOriTailN, uint32_t pltTailN)
{
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (IsSameType<T2, bfloat16_t>::value) {
            ...
            AscendC::MicroAPI::MaskReg preg_all_b8_128 = AscendC::MicroAPI::UpdateMask<T2>(maskLen);
            ...
            AscendC::MicroAPI::Or((AscendC::MicroAPI::RegTensor<uint8_t>&)vreg_exp_merge_tmp_hifp8,
                (AscendC::MicroAPI::RegTensor<uint8_t>&)vreg_exp_even_hifp8,
                (AscendC::MicroAPI::RegTensor<uint8_t>&)vreg_exp_odd_hifp8, preg_all_b8);
    ...
    const T minValue)
{
    ...
    ProcessVec1NoUpdateGeneralImpl128VF<T, T2, s1BaseSize, s2BaseSize>(expUb, expSumUb, maxUb, maxUbStart, srcUb,
        indexesUb, blockStride, repeatStride, m, scale, minValue, pltOriTailN, pltTailN);
}
} // namespace
