...
{
    for (uint16_t i = srcM; i < Ceil<uint16_t>(srcM, 8) * 8; i++) {
        AscendC::Duplicate(spTensor[i * 128], static_cast<T>(0), 64);
        AscendC::Duplicate(dpdsTensor[i * 128], static_cast<T>(0), 64);
    }
