    ...
    ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if (runInfo.s2RealSize > s2SplitSize) {
        CopyToL1Nd2Nz<INPUT_T>(mm1BTensor, GetKeyGm(runInfo, constInfo)[runInfo.keyOffset], s2SplitSize,
        ...
        CopyToL1Nd2Nz<INPUT_T>(mm1BTensor[L1Boffset], GetKeyGm(runInfo, constInfo)[runInfo.keyOffset + gmSplitOffset],
                                runInfo.s2RealSize - s2SplitSize, constInfo.dSize, constInfo.mm1Kb);
    } else {
    ...
    for (int n = 0; n < nLoop; n++) {
        ...
        fixpipeParamsVec1.dstStride = fixpipeParamsVec1.mSize * 16; // mmResUb上两行之间的间隔，单位：element。
        ...
        Fixpipe<half, T, FA_CFG_NZ_UB>(outputBuf.template GetTensor<half>()[s2SplitSize * (s1BaseSize >> 1) * n],
